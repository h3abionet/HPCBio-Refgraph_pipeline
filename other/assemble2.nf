...
process megahit_assemble {
    ...
    output:
    tuple val(name), val("megahit"), file("${name}/final.contigs.fa") into rename_ch
    file("${name}/*")
...
process masurca_assemble {
    ...
    output:
    tuple val(name), val("masurca"), file("${name}/CA/final.genome.scf.fasta") into rename_ch
    file("${name}/*")
...
process assembly_rename {
    ...
    input:
    tuple val(name), val(assembler), file(assembly) from rename_ch
