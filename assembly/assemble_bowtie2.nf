...
/* Alignment */
params.bowtie                = true           /*Whether  to align using bowtie2 or use original cram file that are aligned using bwa-mem*/
...
CRAM_Ch2 = Channel.fromFilePairs("${params.samplePath}", size: 1)
...
//if (params.bowtie == 'true') {
    
process convert_cram_fastq {
 tag                      { id }
    executor               myExecutor
    cpus                   6
    queue                  params.myQueue
    memory                 "$defaultMemory GB"
    module                 "SAMtools/1.12-IGB-gcc-8.2.0"
    publishDir             "${resultsPath}/Read-Prep/Bowtie/cram-to-fastq",mode:"copy", overwrite: true

   
    input:
    tuple val(id), file(CRAM2) from CRAM_Ch2
    file genome from genome_file
...
process bowtie_index {
    tag                    {"BowtieIndex:${genome}"}
    executor               myExecutor
    cpus                   6
    queue                  params.myQueue
    memory                 "$defaultMemory GB"
    module                 "Bowtie2/2.4.2-IGB-gcc-8.2.0"
    ...
    output:
    //tuple val(GRCH38_decoy_hla), file("GRCH38_decoy_hla*") into bowtie2_index
    file "GRCH38_decoy_hla*" into bowtie2_index, bowtie2_index2

    script:
    """
    bowtie2-build --threads ${task.cpus} ${genome1} ${genome1.getSimpleName()}
    """
...
process align_bowtie {
    ...
    publishDir             "${resultsPath}/Read-Prep/Bowtie/Aligned",mode:"copy", overwrite: true

    input:
    tuple val(id), file(converted_fastq) from cram_converted_fastq
    tuple val(idxname), file(bowtie_indx) from bowtie2_index
    //file bowtie_index from bowtie2_index
    file genome1 from genome_file1
    
    output:
    tuple val(id), file("${id}.bowtie.pe.bam") into bowtie_aligned
    
    script:
    """
    # PE
    bowtie2 -p ${task.cpus} \
    -x ${genome_file1.getParent()}/${idxname.getSimpleName()} \
    -1 ${converted_fastq[0]} -2 ${converted_fastq[1]} | \
    samtools view -bS - > ${id}.bowtie.pe.bam
...
process extract_improper {
    ...
    input:
    tuple val(id), file(aligned) from bowtie_aligned
    file genome from genome_file
    tuple val(idxname2), file(bowtie_indx2) from bowtie2_index2
    ...
    samtools view -@ ${task.cpus} -hbt ${genome_file1.getParent()}/${idxname2} -G 2 -o ${id}.improper.bam ${aligned}
    """
}  
